...
<style>
    ...
    section[data-testid="stSidebar"] {
        background: linear-gradient(180deg, #141428 0%, #1E1E3F 100%);
...
if page == "📤 Upload & Process":
    ...
    with st.expander("🧪 Generate Sample Data", expanded=not st.session_state.processed):
        ...
        if generate_btn:
            ...
            with c1:
                st.download_button(
                    "⬇️ Download Sample Registrations CSV",
            ...
            with c2:
                st.download_button(
                    "⬇️ Download Sample Attendance CSV",
    ...
    if st.session_state.reg_raw is not None and st.session_state.att_raw is not None:
        ...
        if st.button("🚀 Run Full Pipeline", use_container_width=True, type="primary"):
            ...
            try:
                # Step 1: Init DB
                progress.progress(5, text="🗃️ Initializing database...")
                reset_db()
                init_db()
        ...
        st.warning("⚠️ Registration CSV is missing. Please upload it above.")
    else:
        st.warning("⚠️ Attendance CSV is missing. Please upload it above.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.processed:
        st.warning("⚠️ Please process the data first on the Upload page.")
    else:
...
# ══════════════════════════════════════════════════════════════════════════════
elif page == "⚠️ Invalid Records":
    st.markdown("# ⚠️ Invalid & Problematic Records")
